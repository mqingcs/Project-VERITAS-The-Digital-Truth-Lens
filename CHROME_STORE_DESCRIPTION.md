...
### 🚨 THE INTERNET IS BROKEN. FIX YOUR FEED.
...
Hover over any highlighted element to reveal a floating, interactive dashboard. This isn't a simple tooltip; it's a deep-dive analysis station with 6 tabs:
1.  **Risk Gauge**: A real-time, animated meter showing the "Misinformation Risk Score" (0-100).
2.  **Fallacies**: A breakdown of exactly *why* an argument is flawed, with academic definitions.
3.  **Claims**: A list of every atomic fact extracted from that section.
4.  **Verification**: The "Evidence Chain." See exactly which sources supported or refuted the claim, with direct links.
5.  **Graph**: A mini-map of the knowledge network.
6.  **Commander**: A context-aware chat to ask specific questions about that paragraph.

#### 🕸️ 3D Knowledge Graph ("God Mode")
Information doesn't exist in a vacuum. It's connected. Click "View Graph" to enter a fully immersive, 3D force-directed **Knowledge Graph**.
*   **Visualize Connections**: See how Claim A links to Entity B and is supported by Source C.
